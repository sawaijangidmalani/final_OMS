...
router.post("/insertCustomerPo", (req, res) => {
  ...
  con.query(
    sqlCustomerPo,
    [customer, date, qtyAllocated, invoice, cost],
    (err, result) => {
      if (err) {
        console.error("Error inserting data into customerpo:", err);
        return res
          .status(500)
          .send("An error occurred while inserting data into customerpo.");
      }
      ...
      con.query(
        sqlRemainingPo,
        [remainingQty, remainingTotalCost, customer],
        (err, result) => {
          if (err) {
            console.error(
              "Error inserting data into remaining_purchase_order:",
              err
            );
            return res
              .status(500)
              .send(
                "An error occurred while inserting data into remaining_purchase_order."
              );
          }
...
router.get("/getRemainingPurchaseOrder", (req, res) => {
  ...
  con.query(sql, (err, results) => {
    if (err) {
      console.error(
        "Error retrieving data from remaining_purchase_order:",
        err
      );

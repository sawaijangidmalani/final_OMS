...
router.post('/insertItems', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: true, message: 'Database query failed', details: error });
    }
});
...
router.delete('/deleteItems', async (req, res) => {
    ...
    if (!id) {
        return res.status(400).json({ error: true, message: 'Please provide the name of the item to delete' });
    }
...
// Route to get item stock master (incomplete functionality)
router.get("/getItemStockMaster", (req, res) => {
    const { name, supplier, category, brand, unit } = req.query; // Use query for GET requests
    ...
    // Implement logic to return item stock master data based on parameters if needed
    res.status(200).json({ message: "Functionality not yet implemented" });
});

// Route to edit item stock
...
// Route to get edit item details
